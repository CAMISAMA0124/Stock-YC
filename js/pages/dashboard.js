    ...
    if (sentList && sentiment.indicators?.length) {
      sentList.innerHTML = sentiment.indicators.map(ind => `
        <div class="s-item">
          ...
          <div class="s-body">
            <div class="s-name">${ind.label} <span style="font-size:11px; color:var(--text-3); margin-left:2px; font-weight:normal; cursor:help; padding:4px 8px; border:1px solid rgba(255,255,255,0.05); border-radius:4px" onclick="event.stopPropagation(); alert(\`${ind.label.replace(/'/g, "\\'")}\\n\\n${ind.desc.replace(/'/g, "\\'")}\`)">ⓘ</span></div>
            <div class="s-bar">
